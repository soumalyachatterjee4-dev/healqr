...
import { collection, getDocs, query, orderBy, limit, where, Timestamp, addDoc, doc, getDoc, serverTimestamp } from 'firebase/firestore';
import { db } from '../lib/firebase/config';
import { TrendingUp, Download, Calendar, MapPin, Stethoscope, Pill, Activity, RefreshCw, FileText, ChevronDown, ChevronRight, Filter, User, History } from 'lucide-react';
...
export default function AdminRxTrends({ adminEmail }: { adminEmail?: string }) {
    ...
    try {
      ...
      if (adminEmail) {
        ...
        if (adminDoc.exists() && adminDoc.data().name) {
          ...
          // Fallback to adminProfiles/super_admin
          const profileDoc = await getDoc(doc(db, 'adminProfiles', 'super_admin'));
          if (profileDoc.exists()) {
            setAdminName(profileDoc.data().name || '');
          }
        }
      }

      // Load extraction logs (last 20)
      try {
        const logsSnap = await getDocs(
          query(collection(db, 'rxExtractionLogs'), orderBy('extractedAt', 'desc'), limit(20))
        );
        setExtractionLogs(logsSnap.docs.map(d => {
          const data = d.data();
          return {
            id: d.id,
      ...
      } catch { /* rxExtractionLogs may not exist yet */ }
    } catch (err) {
  ...
  const exportCSV = async (dataToExport?: RawMoleculeData[]) => {
    ...
    // Add generated-by header to CSV
    const generatedByLine = `# Generated by: ${adminName || adminEmail || 'Admin'} | Date: ${new Date().toLocaleString('en-IN')} | Records: ${rows.length}`;
    const csv = [generatedByLine, headers.join(','), ...rows].join('\n');
    ...
    // Log extraction to Firestore
    try {
      await addDoc(collection(db, 'rxExtractionLogs'), {
        adminName: adminName || 'Unknown',
        adminEmail: adminEmail || '',
        recordCount: rows.length,
        dateRange: fromDate && toDate ? `${fromDate} to ${toDate}` : 'All time',
        filters: [filterState !== 'all' ? `State: ${filterState}` : '', filterSpecialty !== 'all' ? `Specialty: ${filterSpecialty}` : ''].filter(Boolean).join(', ') || 'None',
        extractedAt: serverTimestamp(),
      });
      // Refresh logs
      loadData();
    } catch (err) {
      console.warn('Could not log extraction:', err);
    }
  };
          ...
          <div className="mt-8 bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <div className="space-y-2">
              {extractionLogs.map(log => (
                <div key={log.id} className="flex items-center gap-4 p-3 bg-gray-800/50 rounded-lg">
                  <div className="w-8 h-8 rounded-full bg-amber-500/20 flex items-center justify-center flex-shrink-0">
                    <User className="w-4 h-4 text-amber-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white">
                      <span className="font-medium text-amber-400">{log.adminName}</span>
                      {' '}extracted {log.recordCount} records
                    </p>
                    <p className="text-xs text-gray-500">
                      {log.dateRange} • {log.filters || 'No filters'} • {log.extractedAt.toLocaleString('en-IN')}
                    </p>
                  </div>
