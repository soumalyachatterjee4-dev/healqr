...
import {
  ...
  Check, X, ChevronDown, ChevronUp, Calendar, Download, Filter,
...
const InventoryManager = ({ onMenuChange, onLogout, activeAddOns }: InventoryManagerProps) => {
  ...
  useEffect(() => { if (userId) { loadItems(); loadLogs(); } }, [userId]);

  const loadItems = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, `doctors/${userId}/inventory`), orderBy('name'));
  ...
  const loadLogs = async () => {
    try {
      const q = query(collection(db, `doctors/${userId}/stockLogs`), orderBy('date', 'desc'));
      const snap = await getDocs(q);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        await updateDoc(doc(db, `doctors/${userId}/inventory`, editingItem.id), payload);
        toast.success('Item updated');
      } else {
        await addDoc(collection(db, `doctors/${userId}/inventory`), { ...payload, createdAt: serverTimestamp() });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, `doctors/${userId}/inventory`, id));
  ...
  const handleStockUpdate = async () => {
    ...
    try {
      await updateDoc(doc(db, `doctors/${userId}/inventory`, item.id), {
        quantity: newQty,
      ...
      await addDoc(collection(db, `doctors/${userId}/stockLogs`), {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="lg:ml-64">
        {/* HEADER */}
        <header className="sticky top-0 z-20 px-4 py-3 flex items-center gap-3 bg-slate-900/80 backdrop-blur-xl border-b border-white/5">
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Track all clinic supplies & consumables</p>
            </div>
          </div>
          <button onClick={() => { resetForm(); setShowAddModal(true); }}
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl text-sm font-semibold hover:opacity-90 transition">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            {[
              { label: 'Total Items', value: stats.totalItems, icon: Package, color: 'from-blue-500/20 to-cyan-500/20', text: 'text-blue-400' },
              { label: 'Low Stock', value: stats.lowStock, icon: TrendingDown, color: 'from-yellow-500/20 to-amber-500/20', text: 'text-yellow-400' },
              { label: 'Out of Stock', value: stats.outOfStock, icon: AlertTriangle, color: 'from-red-500/20 to-pink-500/20', text: 'text-red-400' },
              { label: 'Expiring Soon', value: stats.expiringSoon, icon: Calendar, color: 'from-purple-500/20 to-violet-500/20', text: 'text-purple-400' },
              { label: 'Stock Value', value: `₹${stats.totalValue.toLocaleString('en-IN')}`, icon: ShoppingCart, color: 'from-emerald-500/20 to-green-500/20', text: 'text-emerald-400' },
              { label: 'Total Purchased (Debit)', value: `₹${stats.totalDebit.toLocaleString('en-IN')}`, icon: TrendingDown, color: 'from-rose-500/20 to-red-500/20', text: 'text-rose-400' },
              { label: 'Total Used (Credit)', value: `₹${stats.totalCredit.toLocaleString('en-IN')}`, icon: Archive, color: 'from-sky-500/20 to-blue-500/20', text: 'text-sky-400' },
            ].map((s, i) => (
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                ...
                className="w-full pl-10 pr-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder-slate-500 focus:outline-none focus:border-amber-500/50" />
            </div>
            ...
            <div className="flex gap-1 bg-white/5 rounded-xl p-0.5">
              {(['all', 'low', 'out', 'expiring'] as const).map(f => (
                <button key={f} onClick={() => setFilterStatus(f)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${filterStatus === f ? 'bg-amber-500 text-white' : 'text-slate-400 hover:text-white'}`}>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-slate-400">{items.length === 0 ? 'No items yet. Add your first inventory item.' : 'No items match filters.'}</p>
            </div>
          ) : (
                  ...
                  {filtered.map(item => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-white/5 transition">
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-0.5 rounded-md bg-white/5 text-slate-300 text-xs">{item.category}</span>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          {item.expiryDate ? (
                            <span className={`text-xs ${isExpired ? 'text-red-400 font-bold' : isExpiring ? 'text-amber-400' : 'text-slate-400'}`}>
                              {isExpired ? '⛔ ' : isExpiring ? '⚠️ ' : ''}{item.expiryDate}
                            </span>
                          ) : <span className="text-slate-600 text-xs">N/A</span>}
                        </td>
                        <td className="px-4 py-3 text-slate-300 text-xs">₹{item.costPerUnit}/{item.unit}</td>
          ...
          {items.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
                ...
                <p className="text-[11px] text-slate-400 mb-2">Select an item to add newly purchased stock</p>
              ...
              <div className="rounded-xl border border-orange-500/20 bg-orange-500/5 p-4">
                ...
                <p className="text-[11px] text-slate-400 mb-2">Select an item to record usage or consumption</p>
                <div className="flex flex-wrap gap-2">
                  ...
                  {items.filter(i => i.quantity > 0).length === 0 && (
                    <p className="text-xs text-slate-500">All items are out of stock</p>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="max-h-48 overflow-y-auto divide-y divide-white/5">
                {stockLogs.slice(0, 15).map(log => (
                  <div key={log.id} className="px-4 py-2 flex items-center gap-3 text-xs">
                    ...
                    <span className="text-slate-600">{new Date(log.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={resetForm}>
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-5 space-y-3" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-white">{editingItem ? 'Edit Item' : 'Add Inventory Item'}</h2>
              <button onClick={resetForm} className="p-1.5 rounded-lg bg-white/5 text-slate-400 hover:text-white"><X className="w-4 h-4" /></button>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <label className="text-xs text-slate-400 mb-1 block">Item Name *</label>
                <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50" placeholder="e.g., Cotton Rolls" />
              </div>
              <div>
              ...
              <div>
                <label className="text-xs text-slate-400 mb-1 block">Current Qty</label>
                <input type="number" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: e.target.value }))}
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50" placeholder="0" />
              </div>
              <div>
                ...
                <input type="number" value={form.minStock} onChange={e => setForm(f => ({ ...f, minStock: e.target.value }))}
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50" placeholder="5" />
              </div>
              <div>
                <label className="text-xs text-slate-400 mb-1 block">Cost/Unit (₹)</label>
              ...
              <div>
                ...
                <input type="date" value={form.expiryDate} onChange={e => setForm(f => ({ ...f, expiryDate: e.target.value }))}
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50" />
              </div>
              <div className="col-span-2">
                <label className="text-xs text-slate-400 mb-1 block">Supplier</label>
                <input value={form.supplier} onChange={e => setForm(f => ({ ...f, supplier: e.target.value }))}
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50" placeholder="Supplier name" />
              </div>
              <div className="col-span-2">
                <label className="text-xs text-slate-400 mb-1 block">Notes</label>
                <input value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50" placeholder="Optional notes" />
              </div>
            </div>
            <div className="flex gap-2 pt-2">
              <button onClick={resetForm} className="flex-1 py-2 rounded-xl bg-white/5 text-slate-300 text-sm hover:bg-white/10 transition">Cancel</button>
              <button onClick={handleSave} className="flex-1 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white text-sm font-semibold hover:opacity-90 transition">
      ...
      {showStockModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowStockModal(null)}>
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-sm p-5 space-y-3" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs text-slate-400">Current: {showStockModal.item.quantity} {showStockModal.item.unit} • Cost/unit: ₹{showStockModal.item.costPerUnit}</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-slate-400 mb-1 block">Quantity *</label>
                <input type="number" value={stockQty} onChange={e => setStockQty(e.target.value)} autoFocus
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50" placeholder="Qty" />
              </div>
              <div>
            ...
            <div>
              ...
              <input value={stockNote} onChange={e => setStockNote(e.target.value)}
                className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50" placeholder="e.g., Monthly restock" />
            </div>
            <div className="flex gap-2 pt-1">
              <button onClick={() => setShowStockModal(null)} className="flex-1 py-2 rounded-xl bg-white/5 text-slate-300 text-sm">Cancel</button>
