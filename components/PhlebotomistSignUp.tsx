...
import { Mail, ArrowLeft, Syringe, User, Phone, MapPin, Loader2, CheckCircle2 } from 'lucide-react';
...
interface PhlebotomistSignUpProps {
  onBack: () => void;
  onLogin: () => void;
}

export default function PhlebotomistSignUp({ onBack, onLogin }: PhlebotomistSignUpProps) {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      const existing = await getDocs(query(collection(db, 'phlebotomists'), where('email', '==', email.toLowerCase().trim())));
      if (!existing.empty) {
      ...
      // Check by phone too
      const existingPhone = await getDocs(query(collection(db, 'phlebotomists'), where('phone', '==', phone.trim())));
      ...
      // Create pending signup in phlebotomists collection
      await addDoc(collection(db, 'pending_phlebo_signups'), {
      ...
      const actionCodeSettings = {
        url: `${window.location.origin}/verify-login?email=${encodeURIComponent(email.toLowerCase().trim())}&type=phlebo&signup=true`,
    ...
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <p className="text-gray-500 text-sm">Click the link in the email to complete your registration.</p>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-zinc-900 rounded-2xl p-8 border border-zinc-800">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold mb-2">Phlebotomist Sign Up</h1>
            <p className="text-gray-400">Register as a sample collection professional</p>
          </div>
