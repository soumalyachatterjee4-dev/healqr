...
interface ClinicReferralNetworkProps {
...
export default function ClinicReferralNetwork({ onMenuChange, onLogout }: ClinicReferralNetworkProps) {
  const [sidebarOpen, setSidebarOpen] = useState(false);

  // Resolve clinic ID
  const isLocationManager = localStorage.getItem('healqr_location_manager') === 'true';
  const isAssistant = localStorage.getItem('healqr_assistant_mode') === 'true';
  ...
  useEffect(() => {
    ...
    const loadClinic = async () => {
      try {
        ...
        if (snap.exists()) {
          setClinicName(snap.data().name || 'Clinic');
        }
      } catch {}
    };
    loadClinic();
  ...
  useEffect(() => {
    if (!clinicId || !db) return;
    const q = query(collection(db!, 'referrals'), where('fromDoctorId', '==', clinicId), orderBy('createdAt', 'desc'));
  ...
  useEffect(() => {
    if (!clinicId || !db) return;
    const q = query(collection(db!, 'referrals'), where('toDoctorId', '==', clinicId), orderBy('createdAt', 'desc'));
    const unsub = onSnapshot(q, (snap) => {
  ...
  const sendReferral = async () => {
    ...
    try {
      await addDoc(collection(db!, 'referrals'), {
        fromDoctorId: clinicId,
        fromDoctorName: clinicName,
        fromDoctorSpecialty: 'Clinic',
          ...
          {activeTab === 'referrers' && (
            <div className="space-y-3">
              <p className="text-xs text-gray-500">
                Patients referred to your clinic by external referrers (pharmacists, agents, etc.)
