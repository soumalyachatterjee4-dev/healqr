...
import { Mail, ArrowLeft, Syringe } from 'lucide-react';
...
interface PhlebotomistLoginProps {
  onBack: () => void;
  onSignUp: () => void;
  onSuccess?: () => void;
}

export default function PhlebotomistLogin({ onBack, onSignUp, onSuccess }: PhlebotomistLoginProps) {
  ...
  const handleLogin = async () => {
    ...
    try {
      const actionCodeSettings = {
        url: `${window.location.origin}/verify-login?email=${encodeURIComponent(email.toLowerCase().trim())}&type=phlebo`,
      ...
      setLinkSent(true);
      toast.success('Login Link Sent', { description: 'Check your email to log in.' });
    } catch (error: any) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-zinc-900 rounded-2xl p-8 border border-zinc-800">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold mb-2">Phlebotomist Login</h1>
            <p className="text-gray-400">Access your collection dashboard</p>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              ...
              <Input value={email} onChange={(e) => setEmail(e.target.value)}
                className="pl-12 bg-black border-zinc-800 text-white h-14 rounded-lg focus:border-teal-500"
                placeholder="your@email.com" type="email" />
            </div>
          </div>

          <Button onClick={handleLogin} disabled={loading}
            className="w-full bg-teal-600 hover:bg-teal-700 text-white h-14 rounded-lg text-lg font-medium mb-6">
