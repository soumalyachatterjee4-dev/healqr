...
import {
  Menu, IndianRupee, Plus, Trash2, Download, Share2,
  Calendar, Search, User, FileText, ChevronLeft, ChevronRight,
  Phone, Receipt, X, Check
} from 'lucide-react';
...
interface ClinicBillingReceiptProps {
...
export default function ClinicBillingReceipt({ onMenuChange = () => {}, onLogout, activeAddOns = [] }: ClinicBillingReceiptProps) {
  ...
  const [clinicInfo, setClinicInfo] = useState<{
    ...
    clinicId: string; profileSlug: string;
    allBranches: Array<{ name: string; address: string }>;
  }>({ name: '', address: '', phone: '', clinicId: '', profileSlug: '', allBranches: [] });
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          setClinicInfo({
            ...
            address: d.address || locations[0]?.address || '',
            phone: d.phone || d.mobile || '',
            clinicId,
            profileSlug: d.profileSlug || '',
            allBranches: locations.map((l: any) => ({ name: l.name || '', address: l.address || '' })),
          });
        }
      } catch {}
    };
  ...
  const loadReceipts = async () => {
    ...
    try {
      const q = query(
        collection(db, 'clinics', clinicId, 'receipts'),
  ...
  const generateReceiptNo = () => {
    ...
    receiptCounter++;
    return `HQC-${dateStr}-${String(savedReceipts.length + receiptCounter).padStart(3, '0')}`;
  };
  ...
  const handleGenerateReceipt = async () => {
    ...
    try {
      // Save to Firestore
      if (clinicId && db) {
        await addDoc(collection(db, 'clinics', clinicId, 'receipts'), {
          date: selectedDate,
          receiptNo,
      ...
      // === HEADER: Clinic Name & Details ===
      pdfDoc.setFontSize(22);
      pdfDoc.setTextColor(40, 40, 40);
      pdfDoc.setFont('helvetica', 'bold');
      pdfDoc.text(clinicInfo.name.toUpperCase(), margin, y);
      y += 8;
      ...
      if (clinicInfo.address) {
        const splitAddr = pdfDoc.splitTextToSize(clinicInfo.address, pageWidth / 2 - margin);
        pdfDoc.text(splitAddr, margin, y);
        y += splitAddr.length * 4;
      }
      if (clinicInfo.phone) { pdfDoc.text(`Ph: ${clinicInfo.phone}`, margin, y); y += 5; }
      ...
      try {
        ...
        const qrUrl = clinicInfo.profileSlug
          ? `https://healqr.com/clinic/${clinicInfo.profileSlug}`
          : `https://teamhealqr.web.app/?clinicId=${clinicId}`;
        const qrDataUrl = await QRCode.toDataURL(qrUrl, { margin: 1, width: 200 });
        pdfDoc.addImage(qrDataUrl, 'PNG', qrX, qrY, qrSize, qrSize);
      } catch {}

      // === HEADER DIVIDER ===
      const headerEndY = Math.max(y + 3, 55);
      ...
      const displayDatePdf = new Date(selectedDate + 'T00:00:00').toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
      pdfDoc.text(`Date: ${displayDatePdf}`, pageWidth - margin, y, { align: 'right' });
      ...
      y += 7;
      pdfDoc.setFont('helvetica', 'bold');
      pdfDoc.text('Doctor:', margin, y);
      pdfDoc.setFont('helvetica', 'normal');
      pdfDoc.text(selectedPatient.doctorName || '-', margin + 25, y);
      ...
      if (clinicInfo.allBranches.length > 1) {
        const chLineH = 4;
        const footerBoxH = 5 + clinicInfo.allBranches.length * chLineH + 2;
        pdfDoc.setDrawColor(200, 200, 200);
        pdfDoc.setLineWidth(0.2);
        pdfDoc.setFillColor(248, 250, 252);
        ...
        pdfDoc.setFont('helvetica', 'bold');
        pdfDoc.setTextColor(60, 60, 60);
        pdfDoc.text('OUR BRANCHES', margin + 3, footerStartY + 4);
        pdfDoc.setFont('helvetica', 'normal');
        pdfDoc.setTextColor(80, 80, 80);
        pdfDoc.setFontSize(6.5);
        clinicInfo.allBranches.forEach((br, i) => {
          const brY = footerStartY + 8 + i * chLineH;
          pdfDoc.text(`${br.name} \u2022 ${br.address}`, margin + 5, brY);
        });
      }
      ...
      pdfDoc.text(`Digitally generated by HealQR \u2022 No signature required \u2022 ${displayDatePdf}`, pageWidth / 2, 277, { align: 'center' });
  ...
  const handleWhatsAppShare = () => {
    ...
    const displayDateWa = new Date(selectedDate + 'T00:00:00').toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
    const message = `*Receipt - ${clinicInfo.name}*\nDate: ${displayDateWa}\n\nPatient: ${selectedPatient.patientName}\nDoctor: ${selectedPatient.doctorName}\nMobile: ${selectedPatient.phone}\n\n${itemsText}\n\n*Total: \u20B9${totalAmount.toLocaleString('en-IN')}*\nPayment: ${paymentMethod}\n${remarks ? `Note: ${remarks}\n` : ''}\n_Generated by HealQR_`;
  ...
  const handleDeleteReceipt = async (receiptId: string) => {
    if (!clinicId || !db) return;
    try {
      await deleteDoc(doc(db, 'clinics', clinicId, 'receipts', receiptId));
        ...
        <div className="sticky top-0 z-30 bg-zinc-950/80 backdrop-blur-sm border-b border-zinc-800 px-4 lg:px-8 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-zinc-500 hidden sm:block">{displayDate}{isToday && <span className="text-blue-400 ml-1.5">• Today</span>}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setShowHistory(!showHistory)}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${showHistory ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800 border border-zinc-800'}`}>
              <FileText className="w-3.5 h-3.5" /> History
              {savedReceipts.length > 0 && (
                <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${showHistory ? 'bg-blue-500/30 text-blue-300' : 'bg-zinc-700 text-zinc-300'}`}>
        ...
        <div className="px-4 lg:px-8 py-6 max-w-6xl mx-auto space-y-5">
            ...
            <div className="flex items-center gap-2">
              ...
              {!isToday && (
                <button onClick={() => setSelectedDate(new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().split('T')[0])}
                  className="ml-1 text-[11px] text-blue-500 hover:text-blue-400 px-2.5 py-1 bg-blue-500/10 border border-blue-500/20 rounded-lg font-medium">Today</button>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3">
              ...
              <div className="text-xl font-bold text-blue-400">{savedReceipts.length}</div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                  <FileText className="w-4 h-4 text-blue-400" /> Receipts — {displayDate}
                </h3>
                {savedReceipts.length > 0 && (
                  <span className="text-blue-400 text-xs font-bold">Total: {'\u20B9'}{todayRevenue.toLocaleString('en-IN')}</span>
                )}
              </div>
              {savedReceipts.length === 0 ? (
          ...
          {!selectedPatient ? (
                  ...
                  <div className="p-3">
                    <div className="relative mb-3">
                      ...
                      <input type="text" placeholder="Search name or phone..." value={patientSearch}
                        ...
                        className="w-full bg-black/50 border border-zinc-800 rounded-lg pl-9 pr-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500 transition-colors placeholder:text-zinc-600" />
                    </div>
                    {loading ? (
                            ...
                            <button key={p.bookingId} onClick={() => handleSelectPatient(p)}
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2.5">
                                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${hasReceipt ? 'bg-blue-500/10 text-blue-500' : 'bg-zinc-800 text-zinc-400'}`}>
                                    {hasReceipt ? <Check className="w-3.5 h-3.5" /> : p.patientName.charAt(0).toUpperCase()}
                                  </div>
                                  <div>
                                    <div className="text-white text-sm font-medium flex items-center gap-1.5">
                                      {p.patientName}
                                      {hasReceipt && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-blue-500/20 text-blue-400 font-bold">BILLED</span>}
                                      {p.isSeen && !hasReceipt && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 font-bold">SEEN</span>}
                                    </div>
                                    <div className="text-zinc-500 text-[11px] flex items-center gap-1 mt-0.5">
                                      <Phone className="w-3 h-3 shrink-0" /> {p.phone || 'N/A'}
                                      <span className="text-zinc-600">• Dr. {p.doctorName}</span>
                                    </div>
                                  </div>
                                </div>
                                <div className="text-right shrink-0">
                                  <div className="text-emerald-400 text-sm font-bold">{'\u20B9'}{p.consultationFee || 0}</div>
                                  <div className="text-zinc-600 text-[10px] truncate max-w-[80px]">{p.doctorName}</div>
                                </div>
                              </div>
                            </button>
                          );
              ...
              <div className="lg:col-span-3">
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                  <div className="px-4 py-3 border-b border-zinc-800">
                    <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                      <Receipt className="w-4 h-4 text-blue-400" /> Receipt Builder
                    </h3>
                  </div>
                  <div className="flex flex-col items-center justify-center py-16 px-6">
                    ...
                    {patients.length > 0 && (
                      <div className="mt-4 flex items-center gap-1.5 text-[11px] text-blue-500">
            ...
            <>
                  ...
                  return (
                    <button key={p.bookingId} onClick={() => handleSelectPatient(p)}
                      ...
                      <div className={`w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-bold shrink-0 ${
                        isActive ? 'bg-blue-500/20 text-blue-400' : hasReceipt ? 'bg-blue-500/10 text-blue-500' : 'bg-zinc-800 text-zinc-500'
                      }`}>
                      ...
                      {hasReceipt && <span className="text-[8px] px-1 py-0.5 rounded-full bg-blue-500/20 text-blue-400 font-bold">BILLED</span>}
                    </button>
                  );
                })}
              </div>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                {/* Patient Header */}
                <div className="bg-blue-500/5 border-b border-blue-500/20 px-4 py-2.5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-zinc-400 text-[11px] flex items-center gap-2">
                        ...
                        <span className="text-zinc-600">•</span><span>Dr. {selectedPatient.doctorName}</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button onClick={addLineItem} className="text-[11px] text-blue-500 hover:text-blue-400 flex items-center gap-1 px-2 py-0.5 bg-blue-500/10 rounded-md hover:bg-blue-500/20 transition-colors">
                        ...
                        {lineItems.map((li, idx) => (
                          <div key={li.id} className="flex items-center gap-1.5 group">
                            ...
                            <input type="text" value={li.description}
                              ...
                              className="flex-1 bg-black/50 border border-zinc-800 rounded-md px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-blue-500 transition-colors placeholder:text-zinc-600" />
                            <div className="relative w-20 shrink-0">
                              ...
                              <input type="number" value={li.amount || ''}
                                ...
                                placeholder="0"
                                className="w-full bg-black/50 border border-zinc-800 rounded-md pl-6 pr-2 py-1.5 text-xs text-white text-right focus:outline-none focus:border-blue-500 transition-colors placeholder:text-zinc-600" />
                            </div>
                            {lineItems.length > 1 && (
                      ...
                      <div className="grid grid-cols-2 gap-1.5">
                        {['Injection', 'Dressing', 'ECG', 'Nebulization', 'Vaccination', 'Lab Test', 'Procedure Fee', 'Follow-up'].map(label => (
                          <button key={label} onClick={() => setLineItems(prev => [...prev, { id: crypto.randomUUID(), description: label, amount: 0 }])}
                            className="text-[11px] px-2.5 py-1.5 bg-zinc-800/80 text-zinc-400 rounded-lg hover:bg-blue-500/10 hover:text-blue-400 border border-zinc-800 transition-all text-left">
                            + {label}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="space-y-3">
                      <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl px-4 py-3 flex items-center justify-between">
                        <span className="text-white font-bold text-sm">Total</span>
                        <span className="text-blue-400 font-bold text-xl">{'\u20B9'}{totalAmount.toLocaleString('en-IN')}</span>
                      </div>
                          ...
                          {(['CASH', 'UPI', 'CARD'] as const).map(method => (
                            <button key={method} onClick={() => setPaymentMethod(method)}
                              className={`flex-1 py-1.5 rounded-md text-xs font-medium transition-all ${
                                paymentMethod === method ? 'bg-blue-600 text-white' : 'text-zinc-400 hover:text-white'
                              }`}>
                              {method}
                            </button>
                          ))}
                      ...
                      <div>
                        ...
                        <input type="text" value={remarks} onChange={(e) => setRemarks(e.target.value)}
                          ...
                          className="w-full bg-black/50 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-blue-500 transition-colors placeholder:text-zinc-600" />
                      </div>

                      <div className="flex gap-2">
                        <button onClick={handleGenerateReceipt} disabled={generatingPdf || totalAmount <= 0}
                          className="flex-1 py-2 bg-blue-600 hover:bg-blue-500 text-white font-medium rounded-xl transition-all flex items-center justify-center gap-1.5 text-xs disabled:opacity-40 disabled:cursor-not-allowed">
                          {generatingPdf
