...
import {
  ...
  MapPin, Clock, Phone, Plus, Save, ImageIcon, ToggleLeft, ToggleRight,
} from 'lucide-react';
import { Button } from './ui/button';
import { Input } from './ui/input';

interface ParamedicalPersonalizedTemplatesProps {
  paraId: string;
}
...
interface Camp {
  ...
  location: string;
  pincode?: string;
  services?: string[]; // e.g., 'Free BP check', 'Sample collection'
  contactPhone?: string;
  isActive: boolean;
  createdAt: string;
}

const MAX_TEMPLATE_BYTES = 5 * 1024 * 1024; // 5MB

export default function ParamedicalPersonalizedTemplates({ paraId }: ParamedicalPersonalizedTemplatesProps) {
  ...
  const persistTemplates = async (next: Template[]) => {
    ...
    window.dispatchEvent(new CustomEvent('para-template-refresh'));
  };
  ...
  const handleTemplateUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!auth?.currentUser || auth.currentUser.uid !== paraId) {
      toast.error('Session expired — please re-login'); return;
    }
    if (tab === 'camp') return; // camp banners go through camp form
    ...
    try {
      const ts = Date.now();
      const path = `paramedical-templates/${paraId}/${cat}/${ts}_${file.name}`;
  ...
  const deleteTemplate = async (tpl: Template) => {
    ...
    try {
      // Best-effort storage cleanup (URL → ref)
      try {
        const sref = ref(storage, tpl.imageUrl);
  ...
  const saveCamp = async () => {
    ...
    try {
      ...
      if (campBannerFile) {
        const path = `paramedical-templates/${paraId}/camp/${Date.now()}_${campBannerFile.name}`;
        const sref = ref(storage, path);
  ...
  const deleteCamp = async (c: Camp) => {
    ...
    try {
      if (c.bannerUrl) {
        try { await deleteObject(ref(storage, c.bannerUrl)); } catch {}
      }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 border border-purple-500/30 rounded-xl p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">
            Add a health tip, festival wish, or announce a camp. Visible on your mini-website to every visitor.
          </p>
        </div>
      </div>
      ...
      <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="text-blue-200/90 text-xs leading-relaxed">
            <strong>Display limits:</strong> 1 Health Tip, 1 Festival Wish, multiple camps. Camps with past dates are auto-archived. Toggle off to hide without deleting.
          </div>
        </div>
      </div>
      ...
      {/* Health Tip / Festival Wish (image upload) */}
      {tab !== 'camp' && (
              ...
              {uploading ? (
                ...
                <>
                  ...
                  <div className="text-center">
                    <p className="text-white font-medium">Click to upload {tab === 'health-tip' ? 'a health tip' : 'a festival wish'}</p>
      ...
      {tab === 'camp' && (
        <div className="space-y-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-white font-semibold flex items-center gap-2">
                ...
                {editingCampId ? 'Edit Camp' : 'Announce a Health Camp'}
              </h4>
              {editingCampId && (
                <Button size="sm" variant="ghost" onClick={resetCampForm} className="text-gray-400">Cancel edit</Button>
              )}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Input value={campTitle} onChange={e => setCampTitle(e.target.value)} placeholder="Title (e.g., Free Diabetes Screening Camp)" className="bg-black border-zinc-800 text-white md:col-span-2" />
              ...
              <Input value={campServices} onChange={e => setCampServices(e.target.value)} placeholder="Services (comma-separated, e.g., BP check, Glucose, Sample collection)" className="bg-black border-zinc-800 text-white md:col-span-2" />
            </div>
            ...
            <div>
              ...
              <div className="flex items-start gap-3">
                ...
                {(campBannerPreview || existingBannerUrl) && (
                  <Button size="sm" variant="ghost" onClick={() => { setCampBannerFile(null); setCampBannerPreview(''); setExistingBannerUrl(''); }} className="text-red-400">
                    Remove
                  </Button>
                )}
              </div>
          ...
          {/* Upcoming */}
          <CampList title="Upcoming Camps" camps={upcomingCamps} onEdit={openCampForEdit} onDelete={deleteCamp} onToggle={toggleCamp} onPreview={(u) => setPreview(u)} emptyMsg="No upcoming camps. Announce one above." />

          {/* Past */}
          {pastCamps.length > 0 && (
            <CampList title="Past Camps (archive)" camps={pastCamps} onEdit={openCampForEdit} onDelete={deleteCamp} onToggle={toggleCamp} onPreview={(u) => setPreview(u)} emptyMsg="" muted />
          )}
        </div>
      )}
