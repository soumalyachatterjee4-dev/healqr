...
import { Menu, ArrowLeft, Download, FileText, Filter, Search } from 'lucide-react';
...
export default function ClinicReports({
    ...
    const loadDoctors = async () => {
      ...
      try {
        ...
        if (clinicSnap.exists()) {
          ...
          setLinkedDoctors(doctors.map((d: any) => ({
            id: d.doctorId || d.uid,
            name: d.name,
      ...
      try {
        ...
        const data: ReportData[] = snapshot.docs.map(docSnap => {
          ...
          try {
            ...
            console.warn('Decryption failed for record:', docSnap.id);
          }
          ...
          return {
            ...
            doctorSpecialty: '', // Can be filled if needed
  ...
  const filteredData = useMemo(() => {
    return reportData.filter(item => {
      ...
      // Doctor Filter
      if (selectedDoctor !== 'all' && item.doctorName !== linkedDoctors.find(d => d.id === selectedDoctor)?.name) return false;
      // Booking Type Filter
  ...
  }, [reportData, dateFrom, dateTo, selectedDoctor, selectedBookingType, selectedCancellationType, linkedDoctors]);
  ...
  const handleExport = () => {
    ...
    const headers = ['Booking Date', 'Appointment Date', 'Booking ID', 'Patient Name', 'Age', 'Phone', 'Doctor', 'Chamber', 'Type', 'Status'];
    const csvContent = [
      headers.join(','),
    ...
    link.setAttribute('download', `clinic_report_${dateFrom}_to_${dateTo}.csv`);
      ...
      <header className="bg-black/80 backdrop-blur-md border-b border-white/5 px-4 md:px-8 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold">Clinic Reports</h1>
              <p className="text-xs text-gray-500 font-mono">Build: 2026-02-23_14:05</p>
            </div>
          </div>
        </div>
        <Button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                />
              </div>
              ...
              {/* Doctor Filter */}
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Filter by Doctor</label>
              ...
              <Table>
                <TableHeader className="bg-zinc-900/50 sticky top-0 z-10">
                  <TableRow className="border-white/5 hover:bg-transparent">
                    ...
                    <TableHead className="text-gray-400 text-xs font-bold uppercase py-4">Doctor</TableHead>
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-20">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <span className="text-gray-500 text-sm">Loading clinic records...</span>
                        </div>
                      </TableCell>
                    </TableRow>
                      ...
                      <TableRow key={record.id} className="border-white/5 hover:bg-white/5 transition-colors group">
                        ...
                        <TableCell className="py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-300">Dr. {record.doctorName}</span>
                          </div>
                        </TableCell>
                        <TableCell className="py-4">
                        ...
                        <TableCell className="py-4">
                          <div className="flex flex-col gap-1">
                            <span className={`text-[10px] font-bold px-2 py-1 rounded w-fit ${
                              record.status === 'Seen' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' :
