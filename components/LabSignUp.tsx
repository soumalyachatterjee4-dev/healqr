...
import { Mail, Microscope, MapPin, ArrowLeft, CheckCircle2, QrCode, Loader2, Search, Building2 } from 'lucide-react';
...
interface LabSignUpProps {
  onBack: () => void;
  onLogin: () => void;
}

export default function LabSignUp({ onBack, onLogin }: LabSignUpProps) {
  ...
  const handleRegister = async () => {
    if (!email || !labName || !address || !pinCode || !newLocationLandmark || !acceptedTerms) {
    ...
    try {
      ...
      if (qrType === 'virtual') {
        // Generate Virtual QR from universal pool
      ...
      // Prevent reuse for pre-printed QR
      ...
      localStorage.setItem('healqr_pending_lab_signup', JSON.stringify(signupData));
      localStorage.setItem('healqr_email_for_signin', email);

      // Send Magic Link
      const encodedData = btoa(JSON.stringify(signupData));
      const actionCodeSettings = {
        url: `${window.location.origin}/verify-email?type=lab&data=${encodedData}`,
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-zinc-900 rounded-2xl p-8 md:p-12 border border-zinc-800">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold mb-2">Lab / Diagnostic Center Registration</h1>
            <p className="text-gray-400">Create your diagnostic center account with QR-powered operations</p>
          </div>

          {/* Lab Name */}
          <div className="mb-6">
            <label className="block mb-3">
              Lab / Center Name <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <Microscope className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
              <Input
                value={labName}
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="pl-12 bg-black border-zinc-800 text-white h-14 rounded-lg focus:border-purple-500"
                placeholder="e.g. 45 Park Street, Ground Floor"
              />
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            {pinCode.length === 6 && getStateFromPincode(pinCode) !== 'Unknown' && (
              <div className="mt-2 flex items-center gap-2 bg-purple-500/10 border border-purple-500/30 rounded-lg px-3 py-2">
                <MapPin className="w-4 h-4 text-purple-500 shrink-0" />
                <span className="text-purple-400 text-sm font-medium">State: {getStateFromPincode(pinCode)}</span>
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              ...
              <Input
                ...
                placeholder="e.g. Near City Hospital, Salt Lake"
                className="pl-12 bg-black border-zinc-800 text-white h-14 rounded-lg focus:border-purple-500"
              />
            </div>
            <p className="text-xs text-gray-500 mt-2">
              Helps patients find your lab. More branches can be added later from your dashboard.
            </p>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <div className="flex flex-col items-center gap-2">
                  <QrCode className={`w-6 h-6 ${qrType === 'preprinted' ? 'text-purple-400' : 'text-gray-400'}`} />
                  <span className={`font-semibold ${qrType === 'preprinted' ? 'text-purple-400' : 'text-gray-400'}`}>
                    Pre-Printed QR
                  </span>
              ...
              >
                <div className="flex flex-col items-center gap-2">
                  <QrCode className={`w-6 h-6 ${qrType === 'virtual' ? 'text-purple-400' : 'text-gray-400'}`} />
                  <span className={`font-semibold ${qrType === 'virtual' ? 'text-purple-400' : 'text-gray-400'}`}>
                    Virtual QR
                  </span>
                  <span className="text-xs text-gray-500 text-center">
          ...
          {qrType === 'preprinted' && (
            <>
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    className="h-14 px-5 bg-purple-600 hover:bg-purple-700 text-white rounded-lg shrink-0"
                  >
              ...
              {availableDivisions.length > 1 && (
                <div className="mb-6">
                  ...
                  <select
                    ...
                    className="w-full h-14 px-4 bg-black border border-zinc-800 text-white rounded-lg focus:border-purple-500"
                  >
              ...
              {selectedCompany && (
                <div className="mb-8">
                  <label className="block mb-3">
                    Lab QR Number <span className="text-red-500">*</span>
                  </label>
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-12 bg-black border-zinc-800 text-white h-14 rounded-lg focus:border-purple-500 font-mono"
          ...
          {qrType === 'virtual' && virtualQrGenerated && (
            <div className="mb-8">
              <label className="block mb-3">Generated Virtual QR</label>
              <div className="relative">
                <Input
                  value={qrNumber}
                  readOnly
          ...
          <div className="mb-8">
            <div className="flex items-start gap-3">
              <Checkbox
                ...
                className="mt-0.5 border-zinc-700 data-[state=checked]:bg-purple-500 data-[state=checked]:border-purple-500 flex-shrink-0"
              />
          ...
          <Button
            ...
            disabled={loading}
            className="w-full bg-purple-600 hover:bg-purple-700 text-white h-14 rounded-lg mb-6 text-lg font-medium"
          >
            {loading ? 'Sending Link...' : 'Create Lab Account'}
          </Button>
        ...
        <div className="text-center mt-6">
          <p className="text-gray-400">
            Already have an account?{' '}
            <button onClick={onLogin} className="text-purple-500 hover:underline">
              Log in
            </button>
          </p>
        </div>
