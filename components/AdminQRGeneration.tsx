...
export default function AdminQRGeneration({ onBack }: AdminQRGenerationProps) {
  ...
  const downloadBatchQRs = async (batch: QRBatch, size: string = 'a4') => {
    try {
      ...
      for (let page = 0; page < pages; page++) {
        ...
        ctx.fillText(`Dispatch: ${batch.dispatchDate} | Deadline: ${batch.implementationDeadline}`, pageWidth / 2, 170);
  ...
  const updateBatchStatus = async (batchId: string, newStatus: 'generated' | 'dispatched' | 'deployed') => {
    if (!db) return;
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'generated': return <Package className="w-4 h-4" />;
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1a] text-white p-3 sm:p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <Card className="bg-zinc-900 border-zinc-800 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-amber-400">{stats.dispatched}</p>
              </div>
          ...
          <Card className="bg-zinc-900 border-zinc-800 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-emerald-400">{stats.deployed}</p>
              </div>
              ...
              <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-4 mb-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-emerald-400 font-semibold">Next Available from Universal Pool</p>
                      <p className="text-xs text-gray-400">Includes both pre-printed and virtual QR numbers</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold text-emerald-400 font-mono">HQR{String(nextAvailable).padStart(5, '0')}</p>
                    <Button
                      variant="ghost"
                      size="sm"
                    ...
                    >
                      <RefreshCw className={`w-3 h-3 mr-1 ${checkingNext ? 'animate-spin' : ''}`} />
                      Refresh
                    </Button>
                  </div>
                </div>
        ...
        {activeTab === 'display' && (
          <Card className="bg-zinc-900 border-zinc-800 p-8">
            <h2 className="text-2xl font-bold text-white mb-6">QR Display Panel</h2>
            <p className="text-gray-400 mb-8">Enter a QR number to display and preview the QR code</p>

            {/* Search Input */}
            <div className="max-w-2xl mx-auto mb-8">
              <Label className="mb-3 flex items-center gap-2 text-gray-200 text-lg">
              ...
              <div className="flex gap-3">
                <Input
                  ...
                  placeholder="e.g., HQR0001, HQR1234"
                  className="bg-zinc-950 border-zinc-800 text-white placeholder:text-gray-500 text-lg h-14"
                  onKeyDown={async (e) => {
                    if (e.key === 'Enter') {
                      await searchAndDisplayQR();
                    }
                  }}
                />
                <Button
                  ...
                  className="bg-emerald-500 hover:bg-emerald-600 text-white px-8 h-14"
                >
                  {searchingQR ? 'Searching...' : 'Display QR'}
                </Button>
              </div>
            </div>
            ...
            {displayQRData && displayQRImage && (
              <Card className="bg-zinc-800 border-zinc-700 p-8 max-w-3xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  {displayQRData.dispatchDate && (
                    <div className="bg-zinc-900 p-4 rounded-lg">
                      ...
                      <p className="text-white font-medium">{displayQRData.dispatchDate}</p>
                    </div>
                  )}
                ...
                <Button
                  ...
                  className="w-full mt-6 bg-blue-500 hover:bg-blue-600 text-white h-12"
                >
            ...
            {/* Empty State */}
            {!displayQRData && !searchingQR && (
              <div className="text-center py-16">
                <QrCode className="w-24 h-24 text-gray-600 mx-auto mb-4" />
                <p className="text-gray-400 text-lg">Enter a QR number above to display the QR code</p>
                <p className="text-gray-500 text-sm mt-2">Example: HQR0001, HQR0050, HQR1234</p>
              </div>
            )}
          </Card>
        )}
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-6">
            <Card className="bg-zinc-900 border-zinc-700 p-8 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white">Print Movement Panel</h2>
                <Button
                  variant="outline"
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex flex-col items-center">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold mb-2 ${
                    ...
                    {printProgress === 'downloading' ? '...' : printProgress === 'ready' ? '✓' : '3'}
                  </div>
                  <p className={`text-sm font-medium ${printProgress === 'idle' ? 'text-gray-400' : 'text-blue-400'}`}>
                    Download
                  </p>
                </div>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <p className={`text-sm font-medium ${printProgress === 'ready' ? 'text-emerald-400' : 'text-gray-400'}`}>
                    Print
                  </p>
                </div>
              </div>
              ...
              <Card className="bg-zinc-800 border-zinc-700 p-4 mb-6">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p className="text-white font-medium">{selectedBatch.batchNumber}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Total QRs</p>
                    <p className="text-white font-medium">{selectedBatch.totalQRs} codes</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Region</p>
                    <p className="text-white font-medium">{selectedBatch.region}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">MR Name</p>
                    <p className="text-white font-medium">{selectedBatch.mrName}</p>
                  </div>
                </div>
              </Card>
              ...
              <div className="mb-6">
                <Label className="mb-3 text-gray-200 text-lg">Select Print Size</Label>
  ...
  async function searchAndDisplayQR() {
    ...
    try {
      const qrCollection = collection(db, 'qrPool');
      const q = query(qrCollection, where('qrNumber', '==', displayQRNumber.trim().toUpperCase()));
