...
export default function ClinicLogin({ onBack, onSignUp, onSuccess }: ClinicLoginProps) {
  ...
  const handleLogin = async () => {
    ...
    // DEMO MODE: Simulate login if email contains "demo"
    if (email.toLowerCase().includes('demo')) {
        setTimeout(async () => {
            try {
                await signInWithEmailAndPassword(auth, email, 'HealQRClinicUser2025!');
                ...
                // If user not found or invalid credential, try to create the demo user
                if (e.code === 'auth/user-not-found' || e.code === 'auth/invalid-credential' || e.code === 'auth/invalid-login-credentials') {
                    try {
                        const userCredential = await createUserWithEmailAndPassword(auth, email, 'HealQRClinicUser2025!');
                        const user = userCredential.user;
                        
                        // Create Demo Clinic Document
    ...
    try {
      const actionCodeSettings = {
        url: `${window.location.origin}/verify-login?email=${encodeURIComponent(email)}&type=clinic`,
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-zinc-900 rounded-2xl p-8 border border-zinc-800">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold mb-2">Clinic Login</h1>
            <p className="text-gray-400">Access your clinic dashboard</p>
          </div>
          ...
          <Button 
            onClick={handleLogin}
            disabled={loading}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white h-14 rounded-lg text-lg font-medium mb-6"
          >
